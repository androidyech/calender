...
public class DatePopupWindow extends PopupWindow {
    ...
    private DatePopupWindow(Builder builder) {
        LunarCalendar.init(builder.context);
        ...
        this.setHeight((int) (builder.context.getResources().getDisplayMetrics().heightPixels * 0.85));
    ...
    private void initView() {
        ...
        llEnd = (LinearLayout) rootView.findViewById(R.id.ll_endday);
    ...
    private class DateAdapter extends BaseQuickAdapter<DateInfo, BaseViewHolder> {
        ...
        protected void convert(final BaseViewHolder helper, final DateInfo item) {
            ...
            rv.setLayoutManager(manager);
            final TempAdapter groupAdapter = new TempAdapter(item.getList(),item.getDate());
            ...
            groupAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
                ...
                public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                    ...
                    //重置开始和结束时间，设置开始时间
                    if ( endGroupPosition != -1 && endChildPosition != -1 && startChildPosition != -1 && startGroupPosition != -1) {
                        //重置开始和结束
                        mList.get(startGroupPosition).getList().get(startChildPosition).setStatus(0);
    ...
    private class TempAdapter extends BaseQuickAdapter<DayInfo, BaseViewHolder> {
        String dateStr;
        TempAdapter(@Nullable List<DayInfo> data,String datestr) {
        ...
        protected void convert(BaseViewHolder helper, DayInfo item) {
            ...
            ///////////////////
            if(!name.isEmpty()){
                String list[] = dateStr.replace("年","#").replace("月","#").split("#");
                int year = Integer.parseInt(list[0]);
                int firstM = Integer.parseInt(list[1]);
                int days = Integer.parseInt(name);
                if(!LunarCalendar.gregorianFestival(firstM,days).isEmpty()){
                    helper.setText(R.id.tv_festival, LunarCalendar.gregorianFestival(firstM,days));
                }else {
                    com.atuan.datepickerlibrary.Calendar  lunarCalendar = new com.atuan.datepickerlibrary.Calendar();
                    int[] lunar = LunarUtil.solarToLunar(year, firstM, days);
            ...
            if (status == 0) {
                if (isSelect) {
                    ...
                    ((StateTextView)(helper.getView(R.id.tv_date))).setBg_type(2);
                } else {
                    //没选中状态
                    helper.getView(R.id.tv_date).setVisibility(View.VISIBLE);
                    helper.getView(R.id.tv_status).setVisibility(View.INVISIBLE);
                ...
                helper.getView(R.id.tv_dateDel).setVisibility(View.GONE);
                ((TextView) helper.getView(R.id.tv_status)).setTextColor(activity.getResources().getColor(R.color.blue));
                ((TextView) helper.getView(R.id.tv_date)).setTextColor(activity.getResources().getColor(R.color.white));
                //(helper.getView(R.id.tv_date)).setBackgroundColor(activity.getResources().getColor(R.color.title_bg));
                ((StateTextView)(helper.getView(R.id.tv_date))).setBg_type(1);
                //(helper.getView(R.id.tv_date)).setBackgroundResource(R.drawable.select_bg);
            } else if (status == 2) {
                //结束
                helper.getView(R.id.tv_date).setVisibility(View.VISIBLE);
                ...
                ((TextView) helper.getView(R.id.tv_date)).setTextColor(activity.getResources().getColor(R.color.white));
                //(helper.getView(R.id.tv_date)).setBackgroundColor(activity.getResources().getColor(R.color.title_bg));
                ((StateTextView)(helper.getView(R.id.tv_date))).setBg_type(3);
                //(helper.getView(R.id.tv_date)).setBackgroundResource(R.drawable.select_bg);
            }
            ...
            if (!isSelect && status == 0) {
                if (!isEnable) {
                    ...
                    TextView textView2 = helper.getView(R.id.tv_festival);
                    textView2.setTextColor(activity.getResources().getColor(R.color.a8a8a8));
